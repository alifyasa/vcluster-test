...
const templateEditSchema = z.object({
  ...
  templateId: z.string(),
  newVclusterTemplate: z.string().transform((yamlTmpl) => yamlToJson(yamlTmpl)),
});

async function templateEdit(
  parameters: z.infer<typeof TestActionParametersSchema>
) {
  const { platformHost, loftAccessKey, templateId, newVclusterTemplate } =
    templateEditSchema.parse(parameters);
  const options = {
    method: "PUT",
...
export { templateEdit as templateCreate };
